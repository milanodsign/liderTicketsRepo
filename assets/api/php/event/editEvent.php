    ...
    if (!((strpos($type, "gif") || strpos($type, "jpeg") || strpos($type, "jpg") || strpos($type, "png")) && ($tamano < 2000000))) {
     ...
     - Se permiten archivos .gif, .jpg, .png. y de 200 kb como máximo.</b></div>';
        ...
        if (move_uploaded_file($temp, 'bannerEvents/' . $banner)) {
            ...
            $bannerEvent = '/assets/api/php/event/bannerEvents/' . $banner;
        }
    }
} else {
    $bannerEvent = $_POST['bannerActual'];
}
...
if ($secflyer != '') {
    ...
    if (!((strpos($type, "gif") || strpos($type, "jpeg") || strpos($type, "jpg") || strpos($type, "png")) && ($tamano < 2000000))) {
     ...
     - Se permiten archivos .gif, .jpg, .png. y de 200 kb como máximo.</b></div>';
        ...
        //Si la imagen es correcta en tamaño y tipo
        //Se intenta subir al servidor
        if (move_uploaded_file($temp, 'localLayout/' . $secflyer)) {
            //Cambiamos los permisos del archivo a 777 para poder modificarlo posteriormente
            chmod('localLayout/' . $secflyer, 0777);
            $imgSecFlyer = '/assets/api/php/event/localLayout/' . $secflyer;
        }
    }
} else {
    $imgSecFlyer = $_POST['secflyerActual'];
}

$sql = "UPDATE `eventos` SET `eventType`='" . $eventType . "',`pais`='" . $pais . "',`tipo`='" . $tipo . "',`mayores`='" . $mayores . "',`nomEvent`='" . $nomEvent . "',`categoria`='" . $categoria . "',`artista`='" . $artista . "',`descripcion`='" . $descripcion . "',`fechaIni`='" . $fechaIni . "',`horaIni`='" . $horaIni . "',`fechaFin`='" . $fechaFin . "',`horaFin`='" . $horaFin . "',`region`='" . $region . "',`comuna`='" . $comuna . "',`dir`='" . $dir . "',`lugar`='" . $lugar . "',`flyer`='" . $imgEvnt . "',`banner`='" . $bannerEvent . "', `secflyer`='" . $imgSecFlyer . "' WHERE `id`= '" . $idEvent . "'";
$respuesta = $mysqli->query($sql);

$title = "Edicion de Evento";
$txtSucces = 'El evento fue editado satifactoriamente.';
$txtError = 'El evento no pudo ser editado.';
?>
<!DOCTYPE html>
<html lang="es">
