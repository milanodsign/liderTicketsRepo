...
while ($row = $resultSTS->fetch_array(MYSQLI_ASSOC)) {
    if (isset($row['codTicket'])) {
        $update = 'UPDATE `ticketsSales` SET `name`="' . $name . '", `email`="' . $email . '" WHERE `codTicket`="' . $codTicket . '"';
        $result = $mysqli->query($update);
        if ($result) {
            $sqlTS = 'SELECT * FROM `ticketsSales` WHERE `codTicket`="' . $codTicket . '"';
        ...
        $update = 'UPDATE `courtesyTickets` SET `name`="' . $name . '", `email`="' . $email . '" WHERE `codTicket`="' . $codTicket . '"';
        $result = $mysqli->query($update);
        if ($result) {
            $sqlTS = 'SELECT * FROM `ticketsSales` WHERE `codTicket`="' . $codTicket . '"';
            $resultTS = $mysqli->query($sqlTS);
