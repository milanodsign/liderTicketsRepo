...
$type = $_FILES['bdFile']['type'];
$tamano = $_FILES['bdFile']['size'];
$archivotmp = $_FILES['bdFile']['tmp_name'];
$lineas = file($archivotmp);

$i = 0;

$newNameBD = str_replace(' ', '_', $nameBD);
$exists = $mysqli->query("SELECT 1 FROM `" . $newNameBD . "`");
if ($exists !== true) {
    $query = "CREATE TABLE `" . $newNameBD . "` (
...
$sqlBD = "INSERT INTO `eventosDB`(`id`, `idEvent`, `name`, `cant`) VALUES (NULL,'" . $idEvent . "', '" . $newNameBD . "', '" . count($lineas) . "')";
$resultBD = $mysqli->query($sqlBD);

foreach ($lineas as $linea) {
    $cantRegister = count($lineas);
    $cantAddRegister = $cantRegister - 1;

    if ($i != 0) {
        $datos = explode(";", $linea);

        $name = !empty($datos[0]) ? $datos[0] : '';
        $email = !empty($datos[1]) ? $datos[1] : '';
        ...
        $sqlBDNew = "INSERT INTO `" . $newNameBD . "` (`id`,`name`, `email`) VALUES (NULL,'" . $name . "', '" . $email . "')";
        $resultBDNew = $mysqli->query($sqlBDNew);

        $sql = "INSERT INTO `courtesyTickets`(`id`, `idEvent`, `ticketType`, `ticketCant`, `name`, `email`, `fechaValid`, `horaValid`, `codTicket`, `sendDate`, `sendTime`, `wayPay`) VALUES (null, '" . $idEvent . "', '" . $ticketType . "', 1, '" . $name . "', '" . $email . "', '" . $fechaValid . "', '" . $horaValid . "', '" . $codTicket . "', '" . $sendDate . "', '" . $sendTime . "', 'cortesia')";
                                                    ...
                                                    <tr>
                                                        <td style="padding-bottom:20px;text-align:center">
                                                            <div class="m_809902376527534251cupFoto"
                                                                ...
                                                                <img src="https://lidertickets.cl' . $flayer . '"
...
<body class="registerSuccess">
    ...
    <script>
        ...
        $(document).ready(function() {
            setTimeout(() => {
                location.href = "../../../../pages/courtesyDataBase.php?id=<?php echo $idEvent ?>&nomEvent=<?php echo $nomEvent ?>";
            }, 3000);
        });
    </script>
    <!-- Github buttons -->
