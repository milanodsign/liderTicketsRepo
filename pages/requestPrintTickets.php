<?php
$title = "Solicitudes de impresion de tickets";
session_start();
...
?>
        ...
        <body class="g-sidenav-show   bg-gray-100 ticketsList">
                                    ...
                                    <table class="display requestPrint table-responsive" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">Datos de la Productora</th>
                                                <th scope="col">Nombre del Ticket</th>
                                                <th scope="col">Valor del Ticket</th>
                                        ...
                                        <tbody>
                                            <?php
                                            $sqlRequest = "SELECT * FROM `requestPrintTickets` WHERE `status` = 0";
                                            $resultRequest = $mysqli->query($sqlRequest);
                                            while ($rowRequest = $resultRequest->fetch_array(MYSQLI_ASSOC)) {

                                                $nomEvent = $rowRequest['nomEvent'];
                                                $idT = $rowRequest['ticketType'];

                                                $sqlTT = "SELECT * FROM `ticketsType` WHERE `id`= '" . $idT . "'";
                                                $resultTT = $mysqli->query($sqlTT);
                                                while ($rowTT = $resultTT->fetch_array(MYSQLI_ASSOC)) {
                                                    $ticketType = $rowTT['name'];
                                                }
                                                ...
                                                $price = $rowRequest['priceTicket'];
                                                $cant = $rowRequest['cant'];

                                                if ($rowRequest['productora'] != '' && $rowRequest['numDoc'] != '') {
                                                    $productora = $rowRequest['productora'] . '<span class="d-block" style="font-weight: 600; font-size: 12px">NIT ' . $rowRequest['numDoc'] . ' </span>';
                                                } else {
                                                    $productora = 'No se encontraron datos registrados';
                                                }

                                                switch ($rowRequest['status']) {
                                                    case 0:
                                                        $state = 'En Revision';
                                                        break;
                                                    case 1:
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="../assets/api/php/requestPrintTicket/printTickets.php?id=<?php echo $rowRequest['id'] ?>">
                                                            <button class="btn btn-success btn-lg btnTable" title="Imprimir Ticket">
            ...
            <script>
                ...
                const requestPrintedTicketGo = () => {
                    window.location.href = 'requestPrintedTicket.php';
                }
            </script>
        </body>
