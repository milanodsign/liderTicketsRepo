              ...
              <div class="card">
                ...
                <div class="card-body">
                  <table class="display tabEvent table-responsive nowrap" style="width:100%">
                    ...
                    <tbody>
                      <?php
                      $sqlTC = "SELECT * FROM `courtesyTickets`  WHERE email = '" . $rowUser['mail'] . "' ORDER BY `sendDate` ASC";
                      $resultTC = $mysqli->query($sqlTC);
                      while ($rowTC = $resultTC->fetch_array(MYSQLI_ASSOC)) {

                        $sqlNomEvent = "SELECT * FROM `eventos` WHERE `id`= " . $rowTC['idEvent'];
                      ...
                      ?>
                        <tr>
                          ...
                          <td><?php echo $rowTC['sendDate'] ?></td>
                          <td><?php echo 'Cortesía' ?></td>
                          <td><?php echo $rowTC['codTicket'] ?></td>
                          <td class="botonera">
                            <button class="btn btn-success btn-lg btnTable" title="Ver Ticket" onclick="viewTicket('<?php echo $rowTC['codTicket'] ?>')">
                              <i class="fa-solid fa-eye"></i>
                            </button>
                            <button class="btn btn-success btn-lg btnTable" title="Reenviar" onclick="reSendTicket('<?php echo $rowTC['codTicket'] ?>')">
